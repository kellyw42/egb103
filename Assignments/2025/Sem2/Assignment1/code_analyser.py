...
def score_correctness(correct, total):
    global all_correct
    assert correct <= total and total == 14
    if correct == total:
        all_correct = True
...
def score_loops(correct_score, total_score):
    assert correct_score <= 3 and total_score == 3
...
all_functions = ['Ball','all_possible_collisions','find_collision_time','find_time_when_closest','find_time_when_first_just_touching','gap_to_ball_at_future_time','get_distance_travelled_and_new_speed','math','new_position','new_velocity','normalize_vector_between_centres','normalized_direction_vector','relative_speed','speed','time_to_stop']

            
expected = {
    'speed': Function(),
    'time_to_stop': Function(uses = ['speed']),
    'get_distance_travelled_and_new_speed': Function(uses = ['speed', 'time_to_stop'], ifStmt=True, complexity=2),
    'normalize_vector':Function(ifStmt=True, complexity=2),
    'new_position':Function(uses = ['normalized_direction_vector']),
    'new_velocity':Function(uses = ['normalized_direction_vector']),
    'move':Function(uses = ['get_distance_travelled_and_new_speed', 'new_position', 'new_velocity']),
    'resolve_ball_ball_collision':Function(uses = ['relative_speed', 'normalize_vector_between_centres']),
    'resolve_ball_cushion_collision':Function(ifStmt=True, complexity=3),
    'position_of_ball_at_future_time':Function(uses = ['get_distance_travelled_and_new_speed', 'new_position']),
    'find_time_when_first_just_touching':Function(Loops = 'WhileLoop', uses = ['gap_to_ball_at_future_time'], complexity=4, ifStmt=True),
    'find_collision_time':Function(ifStmt=True, uses = ['relative_speed', 'event_horizon', 'find_time_when_closest', 'gap_to_ball_at_future_time', 'find_time_when_first_just_touching'], complexity=3),
    'find_next_collisions':Function(Loops = 'ForEachLoop(s)', uses = ['all_possible_collisions', 'find_collision_time'], complexity=5, ifStmt=True),
    'all_stationary': Function(Loops = 'ForEachLoop', complexity=4, ifStmt=True),
}
...
def loops(functions):      
    ...
    def loop_score(function, expected_loops):
        ...
        if totalLoops == 0:
            ...
            return 0.5
        elif totalLoops > 1 and not ('(s)' in expected_loops):
            print(f'Function {function.__name__} does not require more than one loop.')
            return 0.5
        elif expected_loops.startswith('ForEach'):
            if foreachLoops == 0:
                print(f'Function {function.__name__} is expected to include a for each loop.')
                return 0.5
            else:
                return 1    
        elif expected_loops == 'WhileLoop':
            if whileLoops == 0:
                print(f'Function {function.__name__} is expected to include a while loop.')
                return 0.5
            else:
                return 1
        else:
...
def maintainability(functions):
    print('Have you added comments where necessary inside your functions to make them easier to follow and understand?')
    print('(This criteria will be marked manually after submission).')    
...
def correctness(functions, Ball, Cushion):
    correct = 0
    for function in functions:
        if test_correctness.run_tests(function, Ball, Cushion, explain=False):
            print(f'\u2705 Passed {function.__name__}')
            correct += 1
        else:
            print(f'\u274C Failed {function.__name__}')
    sum_score(*score_correctness(correct, len(functions)))

def analyse_other_nonfunctional_criteria(functions, Ball, Cushion):
    ...
    correctness(functions, Ball, Cushion)
    print()
